...
$query = "select * from flight_quotation_master where email_id = '$email_id' ";
	  ...
	  <div class="col-xs-12">
		  ...
		  <table class="table table-hover table-bordered no-marg" id="tbl_tour_list">
		    ...
		    while($row_tours = mysql_fetch_assoc($sq_query)){
			    ...
			    $quotation_cost =  $row_tours['quotation_cost'];
...
function multiple_quotation_mail()
{
	...
	$.ajax({
			type:'post',
			url: base_url+'controller/package_tour/quotation/flight/quotation_email_send.php',
...
function sendOn_whatsapp(base_url, quotation_id_arr){
	$.post(base_url+'controller/package_tour/quotation/flight/quotation_whatsapp.php', {quotation_id_arr : quotation_id_arr},function(link){
