...
$sq_hotel = mysql_fetch_assoc(mysql_query("select hotel_name,taxation_type,service_tax from hotel_master where hotel_id='$id'"));
...
//Hotel Image
$sq_singleImage = mysql_fetch_assoc(mysql_query("select hotel_pic_url from hotel_vendor_images_entries where hotel_id='$id'"));
...
$sq_hotel_offer = mysql_query("select * from hotel_offers_tarrif where type='Coupon' and (from_date <='$check_indate' and to_date>='$check_indate') and hotel_id='$id'");
while($row_hotel_offer = mysql_fetch_assoc($sq_hotel_offer)){
...
$sq_update = mysql_query("update b2b_registration set cart_data='$cart_data_arr' where register_id='$register_id'");
