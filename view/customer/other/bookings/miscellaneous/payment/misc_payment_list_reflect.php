...
<table class="table table-bordered bg_white cust_table" id="miscellaneous_payment_list" style="margin:20px 0 !important">
	...
	<tbody>
		<?php 
		$query = "SELECT * from miscellaneous_payment_master where 1";		
		...
		while($row_miscellaneous_payment = mysql_fetch_assoc($sq_miscellaneous_payment)){
			if($row_miscellaneous_payment['payment_amount'] !=0){
			...
			if($row_miscellaneous_payment['clearance_status']=="Pending"){ $bg='warning';
				$sq_pending_amount = $sq_pending_amount + $row_miscellaneous_payment['payment_amount'];
			}
			else if($row_miscellaneous_payment['clearance_status']=="Cancelled"){ $bg='danger';
				$sq_cancel_amount = $sq_cancel_amount + $row_miscellaneous_payment['payment_amount'];
			}		
			$sq_paid_amount = $sq_paid_amount + $row_miscellaneous_payment['payment_amount'];
			...
			$travel_date = 'NA';
			$payment_amount = $row_miscellaneous_payment['payment_amount'];
			...
			<tr class="<?= $bg?>">
				...
				<td class="text-right success"><?= $row_miscellaneous_payment['payment_amount'] ?></td>
				<td>
	...
	<tfoot>
		<tr class="active">
			...
			<th colspan="2" class="warning text-right">Total Pending : <?= number_format($sq_pending_amount,2) ?></th>
			<th colspan="2" class="danger text-right">Total Cancel : <?= number_format($sq_cancel_amount,2) ?></th>
