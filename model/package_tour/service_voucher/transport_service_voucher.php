...
public function transport_service_voucher_save()
{
	...
	if($sq_service_voucher_count==0){
		...
		$sq = mysql_query("insert into package_tour_transport_service_voucher (id, booking_id, special_arrangments, confirm_by, inclusions)  values ('$id', '$booking_id', '$special_arrangments', '$confirm_by', '$inclusions')");
		...
		if($sq){
			for($i=0;$i<sizeof($vehicle_name_array);$i++){
				...
				$sq11 = mysql_query("INSERT INTO `package_tour_transport_voucher_entries`(`entry_id`, `booking_id`, `transport_bus_id`, `driver_name`, `driver_contact`) VALUES ('$entry_id1', '$booking_id','$vehicle_name_array[$i]', '$driver_name_array[$i]','$driver_contact_array[$i]')");
			}
			if($sq11){
		...
		for($i=0;$i<sizeof($vehicle_name_array);$i++){
			$sq = mysql_query("update package_tour_transport_voucher_entries set driver_name='$driver_name_array[$i]',driver_contact='$driver_contact_array[$i]' where booking_id='$booking_id' and transport_bus_id='$vehicle_name_array[$i]'");
		}
		if($sq){
