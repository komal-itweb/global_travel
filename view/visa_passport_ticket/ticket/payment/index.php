...
<div class="row text-right mg_bt_20">
	<div class="col-md-12">
		...
		<button class="btn btn-info btn-sm ico_left" data-toggle="modal" data-target="#ticket_payment_save_modal"><i class="fa fa-plus"></i>&nbsp;&nbsp;Receipt</button>
	</div>
</div>
...
<div class="app_panel_content Filter-panel">
	<div class="row">
		...
		<div class="row text-center">
			<div class="col-xs-12">
				<button class="btn btn-sm btn-info ico_right" onclick="ticket_payment_list_reflect();bank_receipt()">Proceed&nbsp;&nbsp;<i class="fa fa-arrow-right"></i></button>
			</div>
		</div>
</div>

<div id="div_ticket_payment_list" class="main_block loader_parent mg_tp_10">
...
<script>
	...
	function ticket_payment_list_reflect()
	{
		...
		$.post('payment/ticket_payment_list_reflect.php', { customer_id : customer_id, ticket_id : ticket_id, payment_mode : payment_mode, financial_year_id : financial_year_id, payment_from_date : payment_from_date, payment_to_date : payment_to_date, cust_type : cust_type, company_name : company_name, branch_status : branch_status  }, function(data){
			//$('#div_ticket_payment_list').html(data);
			pagination_load(data,columns,true,true,10,'flight_r_book');
			$('.loader').remove();
		});
	}
	...
	function excel_report()
	{
		...
		var company_name = $('#company_filter').val();
		var branch_status = $('#branch_status').val();
		window.location = 'payment/excel_report.php?customer_id='+customer_id+'&ticket_id='+ticket_id+'&from_date='+from_date+'&to_date='+to_date+'&payment_mode='+payment_mode+'&financial_year_id='+financial_year_id+'&cust_type='+cust_type+'&company_name='+company_name+'&branch_status='+branch_status;
	}
...
function whatsapp_send_r(booking_id, payment_amount, base_url){
	$.post(base_url+'controller/visa_passport_ticket/ticket/receipt_whatsapp_send.php',{booking_id:booking_id, payment_amount:payment_amount}, function(data){
		window.open(data);
	});
}
</script>
