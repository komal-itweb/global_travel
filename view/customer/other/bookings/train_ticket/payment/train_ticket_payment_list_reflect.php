...
<table class="table table-bordered cust_table" id="table_train" style="margin:20px 0 !important;">
	...
	<tbody>
		...
		while($row_train_ticket_payment = mysql_fetch_assoc($sq_train_ticket_payment)){
			...
			$payment_amount = $row_train_ticket_payment['payment_amount'];
			...
			<tr class="<?= $bg?>">			
				...
				<td class="text-right success"><?= $row_train_ticket_payment['payment_amount'] ?></td>
				<td>
	...
	<tfoot>
		<tr class="active">
			...
			<th colspan="2" class="active text-right warning">Payment Amount : <?=  number_format(($sq_paid_amount - $sq_pending_amount-$sq_cancel_amount),2); ?></th>
		</tr>
	</tfoot>	
