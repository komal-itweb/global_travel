<?php 
//sale
$sale_total_amount=$sq_booking['net_total'];
...
//Paid
$query = mysql_fetch_assoc(mysql_query("SELECT sum(payment_amount) as sum from train_ticket_payment_master where train_ticket_id='$train_ticket_id' and clearance_status != 'Pending' and clearance_status != 'Cancelled'"));
					...
					<tbody>
						...
						while($row_train_ticket_payment = mysql_fetch_assoc($sq_train_ticket_payment)){
							if($row_train_ticket_payment['payment_amount'] != '0'){
								...
								$sq_customer_info = mysql_fetch_assoc(mysql_query("select * from customer_master where customer_id='$sq_train_info[customer_id]'"));
								...
								<tr class="<?= $bg;?>">				
									...
									<td class="text-right"><?= $row_train_ticket_payment['payment_amount'] ?></td>
								</tr>
							<?php
						 }	
						}
