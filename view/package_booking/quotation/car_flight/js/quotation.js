...
function reflect_feilds() {
	...
	if (type == 'Local') {
		...
		$('#total_max_km,#total_mini_km,#driver_allowance,#permit,#toll_parking,#state_entry,#other_charges,#night_halt,#places_to_visit,#traveling_date').hide();
	}
	if (type == 'Outstation') {
		$('#total_hr,#total_km,#local_places_to_visit').hide();
		$('#total_max_km,#total_mini_km,#driver_allowance,#permit,#toll_parking,#state_entry,#other_charges,#night_halt,#places_to_visit,#traveling_date').show();
	}
}
