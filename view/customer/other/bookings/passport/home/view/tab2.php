<?php 
//sale
$sale_total_amount=$sq_passport_info['passport_total_cost'];
...
//Paid
$query = mysql_fetch_assoc(mysql_query("SELECT sum(payment_amount) as sum from passport_payment_master where passport_id='$passport_id' and clearance_status != 'Pending' and clearance_status != 'Cancelled'"));
							...
							while($row_passport_payment = mysql_fetch_assoc($sq_passport_payment)){
								if($row_passport_payment['payment_amount'] != '0'){
									...
									<tr class="<?= $bg;?>">				
										...
										<td class="text-right"><?= $row_passport_payment['payment_amount'] ?></td>
									</tr>
								<?php
							 }
							}
