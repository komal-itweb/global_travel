...
$query = "select * from package_payment_master where booking_id in (select booking_id from package_tour_booking_master where customer_id='$customer_id') ";
if($booking_id!=""){
  ...
  while($row = mysql_fetch_assoc($sq))
  {
    if($row['amount']!=0)
    {
      ...
      if($row['clearance_status']=="Pending"){ $bg='warning';
        $sq_pending_amount = $sq_pending_amount + $row['amount'];
      }

      if($row['clearance_status']=="Cancelled"){ $bg='danger';
        $sq_cancel_amount = $sq_cancel_amount + $row['amount'];
      }

      if($row['clearance_status']=="Cleared"){ $bg='success';
        $sq_paid_amount = $sq_paid_amount + $row['amount'];
      }
     ...
     ?>         
       <tr class="<?= $bg;?>">
         ...
         <td class="text-right success"> <?php echo $row['amount'] ?> </td>
         <td>
