...
function get_sale_purchase($sale_type)
{
	...
	///Visa Start
		...
		while ($row_visa = mysql_fetch_assoc($sq_visa)) {
			...
			if($sq_visa_entry != $sq_visa_cancel){
				$visa_total_sale += $row_visa['visa_total_cost'] - $service_tax_amount - $markupservice_tax_amount + $credit_charges;
			}	
		}
		...
		while($row_purchase = mysql_fetch_assoc($sq_purchase)){
			$visa_total_purchase += $row_purchase['net_total'];
			$visa_total_purchase -= $row_purchase['service_tax_subtotal'];
		}
	///Visa End
	...
	///Passport Start
		...
		while ($row_forex = mysql_fetch_assoc($sq_passport)) {
			...
			if($sq_passport_entry != $sq_passport_cancel){
				$pass_total_sale += $row_forex['passport_total_cost'] - $service_tax_amount + $credit_charges;
			}
		}
		...
		while($row_purchase = mysql_fetch_assoc($sq_purchase)){
			$pass_total_purchase += $row_purchase['net_total'];
			$pass_total_purchase -= $row_purchase['service_tax_subtotal'];
		}
	///Passport End
	///Excursion Start
		//Sale
		...
		while ($row_exc = mysql_fetch_assoc($sq_exc)) {
			...
			if($sq_exc_entry != $sq_exc_cancel){ 		
				$act_total_sale += $row_exc['exc_total_cost'] - $service_tax_amount - $markupservice_tax_amount + $credit_charges;
			}	
		}
		...
		while($row_purchase = mysql_fetch_assoc($sq_purchase)){
			$act_total_purchase += $row_purchase['net_total'];
			$act_total_purchase -= $row_purchase['service_tax_subtotal'];
		}
	///Excursion End	
	///Forex Start
		//Sale
		...
		while ($row_forex = mysql_fetch_assoc($sq_forex)) {
			...
			$forex_total_sale += $row_forex['net_total'] - $service_tax_amount + $credit_charges;
		}

		//Purchase
		$sq_purchase = mysql_query("select * from vendor_estimate where estimate_type='Forex Booking' and status!='Cancel'");
		while($row_purchase = mysql_fetch_assoc($sq_purchase)){
			$forex_total_purchase += $row_purchase['net_total'];
			$forex_total_purchase -= $row_purchase['service_tax_subtotal'];
		}
	///Forex End
	///Bus Start
		//Sale
		...
		while ($row_exc = mysql_fetch_assoc($sq_exc)) {
			...
			if($sq_exc_entry != $sq_exc_cancel){
				$bus_total_sale += $row_exc['net_total'] - $service_tax_amount - $markupservice_tax_amount + $credit_charges;
			}	
		}
		...
		while($row_purchase = mysql_fetch_assoc($sq_purchase)){
			$bus_total_purchase += $row_purchase['net_total'];
			$bus_total_purchase -= $row_purchase['service_tax_subtotal'];
		}
	///Bus End
	...
	///Hotel Start
		...
		while ($row_exc = mysql_fetch_assoc($sq_exc)) {
			...
			if($sq_exc_entry != $sq_exc_cancel){ 		
				$hotel_total_sale += $row_exc['total_fee'] - $service_tax_amount - $markupservice_tax_amount + $credit_charges;
			}	
		}
		...
		while($row_purchase = mysql_fetch_assoc($sq_purchase)){
			$hotel_total_purchase += $row_purchase['net_total'];
			$hotel_total_purchase -= $row_purchase['service_tax_subtotal'];
		}
	///Hotel End
	///Car Start
		//Sale
...
//Service Tax and Markup Tax
			...
			$car_total_sale += $row_exc['total_fees'] - $service_tax_amount -$markupservice_tax_amount + $sq_paid_amount['sumc'];
		}

		//Purchase
		$sq_purchase = mysql_query("select * from vendor_estimate where estimate_type='Car Rental' and status!='Cancel'");
		while($row_purchase = mysql_fetch_assoc($sq_purchase)){
			$car_total_purchase += $row_purchase['net_total'];
			$car_total_purchase -= $row_purchase['service_tax_subtotal'];
		}
	///Car End
	///Ticket Start
		//Sale
		...
		while ($row_exc = mysql_fetch_assoc($sq_exc)) {
			...
			if($sq_exc_entry != $sq_exc_cancel){
				$ticket_total_sale += $row_exc['ticket_total_cost'] - $service_tax_amount - $markupservice_tax_amount + $credit_card_charges;
			}	
		}
		...
		while($row_purchase = mysql_fetch_assoc($sq_purchase)){
			$ticket_total_purchase += $row_purchase['net_total'];
			$ticket_total_purchase -= $row_purchase['service_tax_subtotal'];
		}
	///Ticket End
	///Train Start
		//Sale
		...
		while ($row_exc = mysql_fetch_assoc($sq_exc)) {
			...
			if($sq_exc_entry != $sq_exc_cancel){
				$train_total_sale += $row_exc['net_total'] - $service_tax_amount + $credit_card_charges;
			}	
		}
		...
		while($row_purchase = mysql_fetch_assoc($sq_purchase)){
			$train_total_purchase += $row_purchase['net_total'];
			$train_total_purchase -= $row_purchase['service_tax_subtotal'];
		}
	///Train End
	...
	///Miscellaneous Start
		...
		while ($row_visa = mysql_fetch_assoc($sq_visa)) {
			...
			if($sq_visa_entry != $sq_visa_cancel){
				$misc_total_sale += $row_visa['misc_total_cost'] - $service_tax_amount - $markupservice_tax_amount + $credit_card_charges;
			}	
		}
		...
		while($row_purchase = mysql_fetch_assoc($sq_purchase)){
			$misc_total_purchase += $row_purchase['net_total'];
			$misc_total_purchase -= $row_purchase['service_tax_subtotal'];
		}
	///Miscellaneous End
