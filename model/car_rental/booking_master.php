...
public function booking_save()
{
  ...
  $total_mini_km = $_POST['total_mini_km'];
  ...
  $sq="INSERT into car_rental_booking( booking_id, customer_id,quotation_id, branch_admin_id,financial_year_id,pass_name, total_pax, days_of_traveling, from_date, to_date, vehicle_name, travel_type, places_to_visit, extra_km,basic_amount,markup_cost,markup_cost_subtotal, service_charge, service_tax_subtotal, total_cost, driver_allowance, permit_charges, toll_and_parking, state_entry_tax,other_charges,night_halt, total_fees, created_at, due_date, emp_id,capacity,total_hrs,total_km,rate,total_max_km,total_mini_km,extra_hr_cost,local_places_to_visit,traveling_date,reflections, roundoff, bsm_values) values ( '$booking_id', '$customer_id','$quotation_id', '$branch_admin_id','$financial_year_id','$pass_name', '$total_pax', '$days_of_traveling','$from_date', '$to_date','$vehicle_name', '$travel_type', '$places_to_visit', '$extra_km','$basic_amount','$markup_cost','$markup_cost_subtotal','$service_charge', '$service_tax_subtotal', '$total_cost', '$driver_allowance', '$permit_charges', '$toll_and_parking', '$state_entry_tax','$other_charges', '$night_halt','$total_fees', '$booking_date', '$due_date','$emp_id','$capacity','$total_hrs','$total_km','$rate','$total_max_km','$total_mini_km','$extra_hr_cost','$local_places_to_visit','$traveling_date','$reflections', '$roundoff', '$bsmValues' )";
  //echo $sq;
...
public function finance_save($booking_id, $payment_id, $row_spec, $branch_admin_id,$particular)
{
  ...
  $car_sale_amount = $other_charges + $driver_allowance + $permit_charges + $toll_and_parking + $state_entry_tax + $actual_cost  + $night_halt;
...
public function finance_update($sq_booking_info, $row_spec,$particular)
{
    ...
    global $transaction_master;
    $car_sale_amount =$night_halt+ $other_charges + $driver_allowance + $permit_charges + $toll_and_parking + $state_entry_tax + $actual_cost;
