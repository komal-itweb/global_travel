...
$objPHPExcel->setActiveSheetIndex(0)
            ...
            ->setCellValue('C2', 'Passport Summary')
...
$count = 0;     
        ...
        while($row_passport = mysql_fetch_assoc($sq_passport)){
            ...
            ->setCellValue('H'.$row_count, get_date_user($row_passport['created_at']))
            ->setCellValue('I'.$row_count, number_format($row_passport['passport_issue_amount'],2))
            ->setCellValue('J'.$row_count, number_format($row_passport['service_charge'],2))
            ->setCellValue('K'.$row_count, number_format($service_tax_amount,2))
            ->setCellValue('L'.$row_count, number_format($credit_card_charges,2))
            ->setCellValue('M'.$row_count, number_format($row_passport['passport_total_cost']+$credit_card_charges,2))
            ->setCellValue('N'.$row_count, number_format($cancel_amount,2))
            ->setCellValue('O'.$row_count, number_format($total_bal,2))
            ->setCellValue('P'.$row_count, number_format($paid_amount+$credit_card_charges,2))
            ->setCellValue('Q'.$row_count, number_format($total_bal-($paid_amount+$credit_card_charges),2))
            ->setCellValue('R'.$row_count, ($row_passport['due_date'] == '1970-01-01') ? 'NA' : get_date_user($row_passport['due_date']))
        ...
        ->setCellValue('Q'.$row_count, 'TOTAL BALANCE :'.number_format($sale_total-$paid_total,2));
...
header('Content-Disposition: attachment;filename="PassportSummary('.date('d-m-Y H:i:s').').xls"');
