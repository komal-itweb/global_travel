...
public function fetch_mobile_no_from_system()
{
	...
	while($row_customer = mysql_fetch_assoc($sq_customer)){
		...
		if($sq_mobile_no_count==0){
			...
			$sq_mobile_no = mysql_query("insert into sms_mobile_no ( mobile_no_id,branch_admin_id, mobile_no, created_at ) values ( '$mobile_no_id', '$branch_admin_id', '$row_customer[contact_no]', '$created_at' )");
			if(!$sq_mobile_no){
				echo "Mobile no saved successfully!";
				exit;
			}
