...
function enquiry_master_save(){
  ...
  $assigned_emp_id= implode(',' , $assigned_emp_id);
  ...
  if($row_count==0){
    ...
    if(!$sq_enquiry){
      ...
      if($by != "cust"){
        $arr_emp_ids = explode(",", $assigned_emp_id);
        for($i=0;$i<sizeof($arr_emp_ids);$i++){
          $this->send_enquiry_mail($enquiry_id, $email_id, $name, $tour_name, $enquiry_specification,$arr_emp_ids[$i],$mobile_no);
        }
