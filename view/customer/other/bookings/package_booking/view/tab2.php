...
<div class="row">
	...
	<div class="col-xs-12 mg_bt_20">
        ...
        <h3 class="editor_title">Train Details
				<?php 
				if($sq_package_info['train_upload_ticket']!=""){
					$newUrl = preg_replace('/(\/+)/','/',$sq_package_info['train_upload_ticket']);
...
<div class="row mg_bt_20">
	...
	<div class="col-xs-12">
		...
		<div class="profile_box main_block">
				...
				<?php 
				if($sq_package_info['plane_upload_ticket']!=""){
					$newUrl = preg_replace('/(\/+)/','/',$sq_package_info['plane_upload_ticket']);
					$newUrl = str_replace("../","", $newUrl);
					$newUrl = BASE_URL.$newUrl;
					?>
						 <a href="<?php echo $newUrl; ?>" class="btn btn-info btn-sm text-right" title="Download Ticket" style="margin-left: 80%!important;" download><i class="fa fa-download"></i></a>
...
<?php $sq_c_count = mysql_num_rows(mysql_query("select * from package_cruise_master where booking_id='$booking_id'")); 
	...
	<div class="col-xs-12 mg_bt_20">
		...
		<div class="profile_box main_block">
				...
				<?php
				if($sq_package_info['cruise_upload_ticket']!=""){
					$newUrl = preg_replace('/(\/+)/','/',$sq_package_info['cruise_upload_ticket']);
					$newUrl = str_replace("../","", $newUrl);
					$newUrl = BASE_URL.$newUrl;
					?>
						 <a href="<?php echo $newUrl; ?>" class="btn btn-info btn-sm text-right" title="Download Ticket" style="margin-left: 80%!important;" download><i class="fa fa-download"></i></a>
