...
public function booking_save(){
  ...
  $bsmValues = json_encode($bsmValues);
	$sq_booking = mysql_query("INSERT INTO hotel_booking_master(booking_id, customer_id, branch_admin_id,financial_year_id,pass_name, adults, childrens, infants, sub_total, service_charge, service_tax_subtotal,markup,markup_tax, discount, tds, total_fee, unique_timestamp, due_date , created_at, emp_id,reflections,bsm_values,roundoff,booking_type,other_comment) VALUES ('$booking_id', '$customer_id', '$branch_admin_id','$financial_year_id','$pass_name', '$adults', '$childrens', '$infants', '$sub_total', '$service_charge', '$service_tax_subtotal', '$markup','$service_tax_markup','$discount', '$tds', '$total_fee', '$unique_timestamp', '$due_date', '$booking_date', '$emp_id','$reflections','$bsmValues','$roundoff','$booking_type','$other_comment')");
	  if(!$sq_booking){
        rollback_t();
