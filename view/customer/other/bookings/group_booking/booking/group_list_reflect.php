	...
	<table class="table table-bordered bg_white cust_table" id="group_table" style="margin: 20px 0 !important;">
		...
		while($row1 = mysql_fetch_assoc($sq1))
		{
			...
			$total_balance += $balance_amount;
		  	...
		  	$count++;
			$bg = ($row1['tour_group_status']=="Cancel") ? "danger" : "";
			?>
			  <tr class="<?= $bg ?>">
			  	...
			  	<td>
			  		<button class="btn btn-info btn-sm" onclick="display_modal(<?php echo $row1['id']; ?>)" title="View Detail"><i class="fa fa-eye"></i></button>
			  	</td>
			  	<td><?= $row1['total_tour_fee'] ?></td>
			    <td><?= $row1['total_travel_expense'] ?></td>
