...
<div class="row">
	<div class="col-md-12 mg_bt_20">
		<div class="profile_box main_block">
    	 	<h3 class="editor_title">Train Details
				<?php
				if($sq_group_info['train_upload_ticket']!=""){
					$newUrl = preg_replace('/(\/+)/','/',$sq_group_info['train_upload_ticket']);
					$newUrl = str_replace("../","", $newUrl);
					$newUrl = BASE_URL.$newUrl;
					?>
						 <a href="<?php echo $newUrl; ?>" class="btn btn-info btn-sm text-right" title="Download Ticket" style="margin-left: 80%!important;" download><i class="fa fa-download"></i></a>
				<?php } ?></h3>
...
<div class="row mg_bt_20">
	<div class="col-md-12">
		<div class="profile_box main_block">
        	 	<h3 class="editor_title">Flight Details
				<?php
				if($sq_group_info['plane_upload_ticket']!=""){
					$newUrl = preg_replace('/(\/+)/','/',$sq_group_info['plane_upload_ticket']);
					$newUrl = str_replace("../","", $newUrl);
					$newUrl = BASE_URL.$newUrl;
					?>
						 <a href="<?php echo $newUrl; ?>" class="btn btn-info btn-sm text-right" title="Download Ticket" style="margin-left: 80%!important;" download><i class="fa fa-download"></i></a>
	                    ...
	                    <tbody>
	                       <?php 
							...
							<tr class="<?php echo $bg; ?>">
							    ...
							    <td><?php echo get_date_user($row_entry['arraval_time']); ?> </td>
							</tr>  
							<script>
								$("#birth_date<?= $offset.$count ?>_d, #expiry_date<?= $offset ?>1").datetimepicker({ timepicker:false, format:'d-m-Y' });
							</script>      
...
<div class="row">
	<div class="col-md-12 mg_bt_20">
		<div class="profile_box main_block">
    	 	<h3 class="editor_title">Cruise Details
				<?php
				if($sq_group_info['cruise_upload_ticket']!=""){
					$newUrl = preg_replace('/(\/+)/','/',$sq_group_info['cruise_upload_ticket']);
					$newUrl = str_replace("../","", $newUrl);
					$newUrl = BASE_URL.$newUrl;
					?>
						 <a href="<?php echo $newUrl; ?>" class="btn btn-info btn-sm text-right" title="Download Ticket" style="margin-left: 80%!important;" download><i class="fa fa-download"></i></a>
                	...
                	<table class="table table-bordered no-marg">
                       ...
                       <?php 
                       	...
                       	?>
							<tr class="<?php echo $bg; ?>">
							    <td><?php echo $count; ?></td>
							    <td><?php echo get_date_user($row_entry['dept_datetime']) ?></td>
							    <td><?php echo get_date_user($row_entry['arrival_datetime']) ?></td>
