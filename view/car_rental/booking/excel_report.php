...
elseif($role!='Admin' && $role!='Branch Admin' && $role_id!='7' && $role_id<'7'){
    ...
    $objPHPExcel->setActiveSheetIndex(0)
            ...
            ->setCellValue('H'.$row_count, "Travelling Date")
    ...
    while($row_booking = mysql_fetch_assoc($sq_booking)){
        ...
        $sq_paid_amount = mysql_fetch_assoc(mysql_query("SELECT sum(`credit_charges`) as sumc from car_rental_payment where booking_id='$row_booking[booking_id]' and clearance_status!='Pending' and clearance_status!='Cancelled'"));
    ...
    $objPHPExcel->setActiveSheetIndex(0)
        ...
        ->setCellValue('H'.$row_count, date('d-m-Y', strtotime($row_booking['traveling_date'])))
        ->setCellValue('I'.$row_count, number_format($row_booking['total_fees'] + $credit_card_charges,2))
...
header('Content-Disposition: attachment;filename="CarRentalBooking('.date('d-m-Y H:i:s').').xls"');
