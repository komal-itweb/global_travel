...

{
	...
	<tr>
	    ...
	    <td><input type="text" id="date_of_journey1" name="date_of_journey" class="app_datetimepicker" placeholder="Journey Date & Time" title="Journey Date & Time" value="<?= date('d-m-Y') ?>"  /></td>
	...
	if($sq_entry_count==0){
		...
		while($row_entry = mysql_fetch_assoc($sq_entry)){
			...
			<tr>
			    ...
			    <td><input type="text" id="date_of_journey<?= $count ?>_u" name="date_of_journey" placeholder="Journey Date" title="Journey Date and Time" class="app_datetimepicker" value="<?= ($row_entry['date_of_journey']=='1970-01-01 05:30:00') ? "" : date("d-m-Y",strtotime($row_entry['date_of_journey'] ))?>"/></td>
