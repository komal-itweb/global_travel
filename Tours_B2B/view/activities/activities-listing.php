...
//City Search
if($city_id!=''){
    $sq_city = mysql_fetch_assoc(mysql_query("select city_name,city_id from city_master where city_id='$city_id'"));
    $query = "select * from excursion_master_tariff where city_id='$city_id'";
}
//Hotel Search
if($activities_id!=''){
    $sq_exc = mysql_fetch_assoc(mysql_query("select entry_id,excursion_name from excursion_master_tariff where entry_id='$activities_id'"));
      ...
      <!-- ***** Activity Listing ***** -->
        ...
        while(($row_query  = mysql_fetch_assoc($sq_query))){
          ...
          if($off_day){
            ...
            //Single Hotel Image
            $sq_singleImage = mysql_fetch_assoc(mysql_query("select * from excursion_master_images where exc_id='$exc_id'"));
            ...
            //Tariff Master 
            $sq_count = mysql_num_rows(mysql_query("select * from excursion_master_tariff_basics where exc_id='$exc_id' and (from_date <='$date1' and to_date>='$date1')"));
            if($sq_count >0){
                $sq_tariff_master = mysql_query("select * from excursion_master_tariff_basics where exc_id='$exc_id' and (from_date <='$date1' and to_date>='$date1')");
                while(($row_tariff_master  = mysql_fetch_assoc($sq_tariff_master))){
                  ...
                  array_push($all_orgcosts_array,array('orgamount' => $total_cost1,'id'=>$currency_id));
                  ...
                  $sq_offers_count = mysql_num_rows(mysql_query("select * from excursion_master_offers where (from_date <='$date1' and to_date>='$date1') and exc_id='$exc_id'"));
                  if($sq_offers_count>0){
                  $row_offers = mysql_fetch_assoc(mysql_query("select * from excursion_master_offers where (from_date <='$date1' and to_date>='$date1') and exc_id='$exc_id'"));
                  ...
                  if($offer_type != ''){
                    if($offer_type == 'Offer'){
                      if($offer_in == 'Percentage'){
                          ...
                          $offer_applied = $offer_amount;
                          $total_cost1 = $total_cost1 - $offer_applied;
                      }
                    }
                  }
                  //Final cost push into array
                  $total_cost = ceil($total_cost1);
                  ...
                  $c_amount = $from_currency_rate / $to_currency_rate * $total_cost1;
                ...
                $prices = $array_master->array_column($all_orgcosts_array,'orgamount');
                $orgmin_array = $all_orgcosts_array[array_search(min($prices), $prices)];
