...
$hotel_results_array= $_POST['data'];
...
?>
    ...
    aria-controls="collapseExample">
        ...
        <div class="dividerSection type-1 noborder">
            ...
            <div class="c-aminityListBlock">
                ...
                <?php } ?>
                <?php if(sizeof($hotel_results_array[$hotel_i]['amenity'])-3 > 0){ ?>
                <li>
                    <div class="amenity st-last">
                    <span class="num">+<?= sizeof($hotel_results_array[$hotel_i]['amenity'])-3 ?></span>
    ...
    <div class="collapse" id="collapseExample<?=$hotel_results_array[$hotel_i]['hotel_id'] ?>">
        ...
        <div class="c-compTabs">
            ...
            $last_date = sizeof($hotel_results_array[$hotel_i]['checkDate_array'])-2;
            $all_costs_array = array();
            $original_costs_array = array();
            for($i=0;$i<sizeof($hotel_results_array[$hotel_i]['final_room_type_array']);$i++){
                if($hotel_results_array[$hotel_i]['final_room_type_array'][$i]['check_date'] == $hotel_results_array[$hotel_i]['checkDate_array'][$last_date]){
                //Total cost
                $room_cost = intval($hotel_results_array[$hotel_i]['final_room_type_array'][$i]['room_cost']) + intval($hotel_results_array[$hotel_i]['final_room_type_array'][$i]['daywise_exbcost'])+ intval($hotel_results_array[$hotel_i]['final_room_type_array'][$i]['child_cost']);
                //Markup
                if($hotel_results_array[$hotel_i]['final_room_type_array'][$i]['markup_type'] == 'Percentage'){
                  $room_cost = $room_cost+($room_cost * ($hotel_results_array[$hotel_i]['final_room_type_array'][$i]['markup_amount']/100));
                }else{
                  $room_cost = $room_cost + ($hotel_results_array[$hotel_i]['final_room_type_array'][$i]['markup_amount']*(sizeof($hotel_results_array[$hotel_i]['checkDate_array'])-1));
                }
                ...
                if($hotel_results_array[$hotel_i]['final_room_type_array'][$i]['offer_type'] != ''){
                    if($hotel_results_array[$hotel_i]['final_room_type_array'][$i]['offer_type'] == 'Offer'){
                        if($hotel_results_array[$hotel_i]['final_room_type_array'][$i]['offer_in'] == 'Percentage'){
                            $text = '%';
                            $room_cost = $room_cost-($room_cost * ($hotel_results_array[$hotel_i]['final_room_type_array'][$i]['offer_amount']/100));
                        }
                        else{
                            $text = '';
                            $room_cost = $room_cost - $hotel_results_array[$hotel_i]['final_room_type_array'][$i]['offer_amount'];
                        }
                    }
            ...
            ?>
                ...
                <!-- ***** Hotel List Card End ***** -->
                <?php } }
                $best_cost = min($all_costs_array);
                $original_cost = min($original_costs_array);
                ?>
                <script>
                setTimeout(function(){
                    ...
                    if('<?= $offer_text ?>' != ''){
                        ...
                        document.getElementById("offer-currency-id<?= $hotel_results_array[$hotel_i]['hotel_id'] ?>").innerHTML= '<?= $h_currency_id ?>';
            ...
            >
                ...
                <!-- **** Amenities **** -->
                <?php if(sizeof($hotel_results_array[$hotel_i]['amenity'])>1){ ?>
                <div class="clearfix margin20-bottom">
                <h3 class="c-heading">
                    Amenities
                </h3>
                <div class="clearfix">
                    ...
                    <?php
                    for($i=0;$i<sizeof($hotel_results_array[$hotel_i]['amenity']);$i++){
                    ?>
                    <script>
                        var ameities3 = getObjects(amenities,'name','<?php echo $hotel_results_array[$hotel_i]['amenity'][$i]; ?>');
                    ...
                    <li class="col-md-4 col-sm-6 col-12">
                        ...
                        <i id="ameities1<?=$i.$hotel_results_array[$hotel_i]['hotel_id']?>"></i><?= $hotel_results_array[$hotel_i]['amenity'][$i] ?>
                        </div>
                    </li>
                    <?php } ?>
                    </ul>
            ...
            >
                ...
                <div class="c-photoGallery js-dynamicLoad">
                    <div class="js-photoGallery owl-carousel">
                        ...
                        $sq_hotelImage = mysql_query("select * from hotel_vendor_images_entries where hotel_id='$thotel_id'");
                        while($sq_singleImage = mysql_fetch_assoc($sq_hotelImage)){
