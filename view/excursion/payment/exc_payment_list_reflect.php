...
$branch_status = $_POST['branch_status'];
		...
		while($row_exc_payment = mysql_fetch_assoc($sq_exc_payment)){
			...
			$payment_id = get_exc_booking_payment_id($row_exc_payment['payment_id'],$year);
			...
			$temp_arr = array( "data" => array(
				...
				$payshow,
				number_format($row_exc_payment['payment_amount']+$row_exc_payment['credit_charges'],2),
				'<a onclick="loadOtherPage(\''. $url1 .'\')" class="btn btn-info btn-sm" title="Download Receipt"><i class="fa fa-print"></i></a>
	
				<button data-toggle="tooltip" class="btn btn-info btn-sm" onclick="exc_payment_update_modal('. $row_exc_payment['payment_id'] .')" title="Update Details"><i class="fa fa-pencil-square-o"></i></button>'
			  ), "bg" =>$bg );
