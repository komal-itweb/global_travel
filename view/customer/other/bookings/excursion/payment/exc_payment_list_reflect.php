...
<table class="table table-bordered bg_white cust_table" id="exc_payment_list" style="margin:20px 0 !important">
	...
	<tbody>
		...
		while($row_exc_payment = mysql_fetch_assoc($sq_exc_payment)){
			...
			$sq_paid_amount = $sq_paid_amount + $row_exc_payment['payment_amount'];

			$payment_id_name = "Excursion Payment ID";
			...
			$booking_name = "Excursion Booking";
			$travel_date = 'NA';
			$payment_amount = $row_exc_payment['payment_amount'];
			...
			$receipt_type = "Excursion Receipt";			
			...
			<tr class="<?= $bg?>">
				...
				<td class="text-right success"><?= $row_exc_payment['payment_amount'] ?></td>
				<td>
