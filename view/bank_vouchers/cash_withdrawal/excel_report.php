...
$count = 0;
    $total_amount=0;
    $sq_deposit = mysql_query($query);
    while($row_deposit = mysql_fetch_assoc($sq_deposit)){

      $sq_bank = mysql_fetch_assoc(mysql_query("select * from bank_master where bank_id='$row_deposit[bank_id]'"));
...
header('Content-Disposition: attachment;filename="Cash_Withdrawal('.date('d-m-Y H:i:s').').xls"');
