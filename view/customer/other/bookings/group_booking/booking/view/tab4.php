<?php
$sale_total_amount=$sq_group_info['total_travel_expense'] + $sq_group_info['total_tour_fee'];
if($sale_total_amount==""){  $sale_total_amount = 0 ;  }

//paid
$query = mysql_fetch_assoc(mysql_query("SELECT sum(amount) as sum from payment_master where tourwise_traveler_id='$sq_group_info[id]' and clearance_status != 'Pending' and clearance_status != 'Cancelled'"));
					 ...
					 while($row_group_payment = mysql_fetch_assoc($sq_group_payment)){
...

					 	{
							...
							<tr class="<?php echo $bg; ?>">
						        ...
						        <td><?php echo $row_group_payment['payment_for']; ?></td>

						        <td><?php echo $row_group_payment['travel_type']; ?></td>

						        <td class="text-right"><?php echo $row_group_payment['amount']; ?></td>
