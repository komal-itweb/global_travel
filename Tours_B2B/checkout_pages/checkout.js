              ...
              coupon_list_arr.every(function(item,i){
                ...
                if(res == 0){
                  ...
                  if(item.offer_in=="Flat"){
                    var new_total_price = parseFloat(grand_total) - parseFloat(item.offer_amount);
                    var text = item.offer_amount+' Off';
                  }else{
                    var new_total_price = parseFloat(grand_total) - (parseFloat(grand_total) * parseFloat(item.offer_amount)/100);
                    var text = parseFloat(item.offer_amount).toFixed(0)+' % Off';
                  }
...
function generate_pdf(type=0){
  ...
  if(type!=0){
	...
	$.post('generate_quotation_pdf.php',{cart_list_arr : cart_list_arr,timing_slots:timing_slots}, function(data){
		$('#pdf_modal').html(data);
  });
}
