...
<form id="frm_tab41_c">
	...
	<div class="col-md-2">
		<small id="basic_show1"><?= ($inclusive_b == '') ? '&nbsp;' : 'Inclusive Amount : <span>'.$inclusive_b ?></span></small>
  		<input type="text" id="subtotal" name="subtotal" placeholder="Basic Cost"  class="text-right form-control" title="Basic Cost" onchange="quotation_cost_calculate1();validate_balance(this.id);get_auto_values('quotation_date','subtotal','payment_mode','service_charge','markup_cost','update','true','basic',true);" value="<?= $basic_cost ?>">  
	...
	<div class="col-md-2 col-sm-6 col-xs-12 mg_bt_10">
    	...
    	<input type="text" name="service_charge" id="service_charge" placeholder="*Service Charge" title="Service Charge" value="<?= $service_charge ?>" onchange="validate_balance(this.id);quotation_cost_calculate1();get_auto_values('quotation_date','subtotal','payment_mode','service_charge','markup_cost','update','false','service_charge')">
	</div>
	<div class="col-md-2">
		<small>&nbsp;</small>
	...
	<div class="col-md-2">
		<small id="markup_show1"><?= ($inclusive_m == '') ? '&nbsp;' : 'Inclusive Amount : <span>'.$inclusive_m ?></span></small>
  		<input type="text" id="markup_cost" name="markup_cost" class="text-right form-control" placeholder="Markup Cost" title="Markup Cost" onchange="quotation_cost_calculate1();validate_balance(this.id);get_auto_values('quotation_date','subtotal','payment_mode','service_charge','markup_cost','update','false','markup');" value="<?= $markup ?>">  
  	</div>
	  <div class="col-md-2 col-sm-6 col-xs-12 mg_bt_10">
	  	<small>&nbsp;</small>
        <input type="text" id="service_tax_markup" name="service_tax_markup" placeholder="*Tax on Markup" title="Tax on Markup" onchange="quotation_cost_calculate1();" value="<?= $sq_quotation['markup_cost_subtotal'] ?>" readonly>
    </div> 
	<div class="col-md-2"> 
		<small>&nbsp;</small>
...
<div class="row mg_bt_10">
	...
	<div class="col-md-2">
		<input type="text"  id="other_charges" name="other_charges" class="text-right form-control" placeholder="Other Charges" title="Other Charges" onchange="quotation_cost_calculate1();validate_balance(this.id)" value="<?php echo $sq_quotation['other_charge']; ?>" > 
	</div>
	<div class="col-md-2">
		<input type="text"  id="night_halt" name="night_halt" class="text-right form-control" placeholder="Night Halt" title="Night Halt" onchange="quotation_cost_calculate1();validate_balance(this.id)" value="<?php echo $sq_quotation['night_halt']; ?>" > 
	</div>
...
function quotation_cost_calculate1(){
	...
	if(night_halt==""||night_halt=='0'){ night_halt = 0;}
...
$('#frm_tab41_c').validate({
	...
	submitHandler:function(form){
		...
		var total_mini_km = $('#total_mini_km').val();
		var subtotal = $('#subtotal').val();
		var markup_cost = $('#markup_cost').val();
		var markup_cost_subtotal = $('#service_tax_markup').val();
