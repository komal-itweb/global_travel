function get_auto_values(booking_date, sub_total, payment_mode, service_charge, markup, type, charges_flag, amount_type, discount, change = false) {
    ...
    get_tax_rules(rules, taxes_result, basic_amount, sub_total, tax_markup, markup, tax_service_charge, service_charge, payment_mode, type, amount_type, discount_amount, charges_flag);
    /////////////////Tax End///////////////////////
    ...
    if (new_taxes_rules.length > 0) {
        ...
        new_taxes_rules && new_taxes_rules.map((rule) => {
            ...
            if (target_amount === 'Service Charge') {
                ...
                var charge_amount = parseFloat(service_charge) + parseFloat(basic_amount) + parseFloat(markup); //FIX THIS ****************
            }
...
function get_tax_rules_on_conditions(final_taxes_rules, basic_amount, payment_mode, type) {
    ...
    if (type === 'save') {
        ...
        var supp_id = $("#sup_id option:selected").text();
        var tour_type = $('#tour_type').val();
    }
    else {
        // var hotel_table = 'tbl_dynamic_visa_update'; 
        var customer = $('#customer_id').val();
        var supp_id = $("#sup_id1 option:selected").text();
