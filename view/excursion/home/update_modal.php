	                       		...
	                       		while($row_entry = mysql_fetch_assoc($sq_entry)){
						            ...
						            }else
						            {
									 ...
									 <tr class="<?= $bg ?>">
									    ...
									    <td><input type="text" id="total_amount-<?= $offset.$count ?>" name="total_amount-<?= $offset.$count ?>" placeholder="Total Amount" title="Excursion Amount" value="<?php echo $row_entry['total_cost'] ?>" onchange="validate_balance(this.id);excursion_amount_calculate(this.id);calculate_exc_expense('tbl_dynamic_exc_booking_update','1');"></td>
