...
function get_car_cost (offset='') {
	...
	$.ajax({
		...
		success  : function (result) {
			...
			$('#total_mini_km'+offset).val(result[0].total_mini_km);
...
function get_enquiry_details(offset)
{
	...
	$.ajax({
		...
		success:function(result){
			...
			$('#other_charges'+offset).val(result[0].other_charges);
			$('#night_halt'+offset).val(result[0].night_halt);
...
function calculate_total_fees(id,offset="")
{
	...
	var night_halt = $('#night_halt'+offset).val();
 ...
 console.log(basic_amount, markupservice_tax_amount , service_tax_amount, markup_cost, service_charge); 
  $('#total_cost'+offset).val(total_cost);
	...
	var total_fees = parseFloat(total_cost) + parseFloat(driver_allowance) + parseFloat(permit_charges) + parseFloat(toll_and_parking) + parseFloat(state_entry_tax)+parseFloat(other_charges)+parseFloat(night_halt);
