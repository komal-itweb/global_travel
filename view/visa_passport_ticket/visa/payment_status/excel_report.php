...
$objPHPExcel->getActiveSheet()->getStyle('B9:C9')->applyFromArray($borderArray); 
        ...
        while($row_visa = mysql_fetch_assoc($sq_visa)){
            ...
            $sq_paid_amount = mysql_fetch_assoc(mysql_query("select sum(payment_amount) as sum,sum(credit_charges) as sumc from visa_payment_master where visa_id='$row_visa[visa_id]' and clearance_status!='Pending' and clearance_status!='Cancelled'"));

            $total_sale=$row_visa['visa_total_cost']+$sq_paid_amount['sumc'];
            ...
            $balance_total = $balance_total + $bal - $cancel_amount;
            ...
            $sq_purchase = mysql_query("select * from vendor_estimate where estimate_type='Visa Booking' and estimate_type_id='$row_visa[visa_id]'");
            ...
            $sq_purchase1 = mysql_fetch_assoc(mysql_query("select * from vendor_estimate where estimate_type='Visa Booking' and estimate_type_id='$row_visa[visa_id]'"));       
...
header('Content-Disposition: attachment;filename="VisaSummary('.date('d-m-Y H:i:s').').xls"');
