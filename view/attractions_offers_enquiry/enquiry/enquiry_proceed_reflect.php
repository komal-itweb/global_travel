...
if($emp_id_filter!=""){
	$enq_count .=" and assigned_emp_id LIKE '%$emp_id_filter%'";
}
...
if($role!='Admin' && $role!='Branch Admin' && $role_id!='7' && $role_id<'7'){
	...
	{
		$enq_count .= " and assigned_emp_id LIKE '%$emp_id%' and enquiry_master.status!='Disabled' ";  
		if($enquiry_type!=""){
			$enq_count .=" and enquiry_type='$enquiry_type' ";
		}
		if($reference_id_filter!=""){
...
if($emp_id_filter!=""){
	$query .=" and assigned_emp_id LIKE '%$emp_id_filter%'";
}
...
if($role!='Admin' && $role!='Branch Admin' && $role_id!='7' && $role_id<'7'){
	$query .=" and assigned_emp_id LIKE '%$emp_id%' ";
	if($enquiry_type!=""){
		$query .=" and enquiry_type='$enquiry_type' ";
	}
	if($reference_id_filter!=""){
...
while($row = mysql_fetch_assoc($sq_enquiries)){
	...
	$sq_emp = mysql_fetch_assoc(mysql_query("select first_name,last_name from emp_master where emp_id in ('$assigned_emp_id')"));
	...
	$followup_datetime=new DateTime($enquiry_status1['followup_date']);
	$datetime_now=new DateTime();
	if($datetime_now > $followup_datetime && $followup_status == 'In-Followup')
	{
		$bg = 'warning';
	}
	if($enquiry_status_filter!=''){
