...
<table class="table table-bordered table-hover cust_table" id="tbl_payment_list" style="margin:20px 0 !important;">
	...
	<tbody>
		...
		while($row_payment = mysql_fetch_assoc($sq_payment))
		{
			...
			$count++;
				...
				$total_paid += $row_payment['payment_amount'];
			...
			<tr class="<?= $bg;?>">				
				...
				<td class="success"><?= $row_payment['payment_amount'] ?></td>				
				<td>
	...
	<tfoot>
		<tr class="active">		
			...
			<th colspan="2" class="success text-right">Payment Amount : <?= number_format(($total_paid-$sq_pending_amount-$sq_cancel_amount),2); ?></th>
		</tr>
	</tfoot>	
</table>
</div>
