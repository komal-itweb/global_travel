...
$hotel_results_array = $_POST['final_arr'];
?>
...
$(document).ready(function () {
    ...
    var label = (<?= sizeof($hotel_results_array)?> === 1||<?= sizeof($hotel_results_array)?> === 0)? 'Hotel result':'Hotels results';
    document.getElementsByClassName("results_count")[0].innerHTML='<?= sizeof($hotel_results_array) ?>'+' '+label;
    document.getElementsByClassName("results_count")[1].innerHTML='<?= sizeof($hotel_results_array) ?>'+' '+label;
    
    var hotel = $('#hotel_name_filter').val();
    // if(hotel != ''){
    //     document.getElementById('result_text').innerHTML = ' Hotel';
    // }
    var hotel_results = $('#hotel_results').val();
    if(hotel_results!=='null'){
        ...
        var html  = '<div class="c-emptyList"><div class="imgDiv"><img src="../../images/search_illustration.svg" alt="" /></div><span class="infoDiv">The Hotels are not found for this search. Please modify search.</span></div>';
