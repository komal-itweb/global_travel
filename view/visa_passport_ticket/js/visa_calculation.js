	...
	if (new_taxes_rules.length > 0) {
		new_taxes_rules &&
			new_taxes_rules.map((rule) => {
				...
				//console.log(charge_amount + 'Charge');
				...
				if (calculation_mode !== '"Exclusive"') {
					...
					if (tax_for === 'service_charge') {
						...
						//console.log(applied_taxes + 'AP TAXES');
...
function get_tax_rules_on_conditions (final_taxes_rules, basic_amount, payment_mode, type) {
	...
	if (type === 'save') {
		...
		var hotel_table = 'tbl_dynamic_visa_update';
						...
						condition.forEach((cond) => {
							...
							switch (condition) {
								case '1':
									...
									if (customer !== '0' && customer !== '') {
										$.ajax({
											...
											global   : false,
											dataType : 'html',
											url      : '../visa/home/inc/get_customer_operator_state.php',
								...
								case '5':
									...
									break;
								case '8':
									if (value == 'Visa' || value == 'All') flag = true;
									break;
								case '3':
									switch (for1) {
										case '!=':
								...
								case '7':
									...
									if (customer !== 0 && customer !== '') {
										$.ajax({
											...
											global   : false,
											dataType : 'html',
											url      : '../visa/home/inc/get_customer.php',
											data     : { customer: customer },
...
function calculate_charges (rules_array, type, basic_amount, markup_amount1) {
	if (rules_array.length) {
		...
		for (var i = 0; i < rowCount; i++) {
			var row = table.rows[i];
			if (row.cells[0].childNodes[0].checked) ++pax_count;
		}
		switch (apply_on) {
			case '1':
				//Per pax
				...
				var service_fee =
						...
						rules_array[0].fee_type === 'Flat' ? parseFloat(rules_array[0].fee) * parseInt(pax_count) :
						parseFloat(target_amount) * parseFloat(rules_array[0].fee) / 100 * parseInt(pax_count);
				return service_fee;
				break;
			case '2':
				//Per Invoice
