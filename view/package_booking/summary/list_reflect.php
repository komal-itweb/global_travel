...
while($row_package = mysql_fetch_assoc($sq_package)){
	...
	$total_paid = 0;
	$sq_paid_amount = mysql_fetch_assoc(mysql_query("SELECT sum(amount) as sum,sum(credit_charges) as sumc from  package_payment_master where booking_id='$row_package[booking_id]' and clearance_status!='Cancelled'"));
	$credit_card_charges = $sq_paid_amount['sumc'];
	$total_paid =  $sq_paid_amount['sum']; 
	if($total_paid == ''){ $total_paid = 0; }
	
	//sale amount
	$tour_fee = $row_package['net_total'] + $credit_card_charges;
	...
	//balance
	$total_balance=$total_amount - $total_paid - $credit_card_charges;	
	...
	$tour_subtotal = $row_package['total_hotel_expense'] - $cancel_tour_amount;
	...
	$temp_arr = array( "data" => array(
		...
		number_format($total_paid+$credit_card_charges,2),
		...
		<a onclick="loadOtherPage(\''. $b_url .'\')" class="btn btn-info btn-sm" data-toggle="tooltip" title="Print Confirmation Form"><i class="fa fa-print"></i></a>'
