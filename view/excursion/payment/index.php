...
<div class="app_panel_content Filter-panel">
	...
	<div class="row text-center mg_tp_10">
		<div class="col-xs-12">
			<button class="btn btn-sm btn-info ico_right" onclick="exc_payment_list_reflect();bank_receipt()">Proceed&nbsp;&nbsp;<i class="fa fa-arrow-right"></i></button>
		</div>
	</div>
</div>

<div id="div_exc_payment_list" class="main_block mg_tp_10">
<div class="table-responsive">
        <table id="exc_r_book" class="table table-hover" style="margin: 20px 0 !important;">         
        </table>
    </div>
</div>
...
<script>
	...
	function exc_payment_list_reflect()
	{
		...
		$.post('payment/exc_payment_list_reflect.php', { customer_id : customer_id, exc_id : exc_id, payment_mode : payment_mode, financial_year_id : financial_year_id, payment_from_date : payment_from_date, payment_to_date : payment_to_date, cust_type : cust_type, company_name : company_name , branch_status : branch_status}, function(data){
			...
			pagination_load(data,columns,true,true,10,'exc_r_book');
			$('.loader').remove();
		});
	}
	...
	$.post(base_url+'controller/excursion/receipt_whatsapp_send.php',{booking_id:booking_id, payment_amount:payment_amount}, function(data){
	window.open(data);
	});
}
</script>
